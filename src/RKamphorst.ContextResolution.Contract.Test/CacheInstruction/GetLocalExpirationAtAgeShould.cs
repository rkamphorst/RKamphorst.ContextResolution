...
public class GetLocalExpirationAtAgeShould
{
    [Theory]
    [InlineData(-1)]
    [InlineData(0)]
    ...
    {
        var expiration =
            CacheInstruction.Transient.GetLocalExpirationAtAge(TimeSpan.FromSeconds(ageSeconds));
    ...
    public void ReturnExpirationMinusAge(int ageSeconds)
    {
        var expiration =
            CacheInstruction.FromTimeSpan(TimeSpan.FromSeconds(600))
                .GetLocalExpirationAtAge(TimeSpan.FromSeconds(ageSeconds));
